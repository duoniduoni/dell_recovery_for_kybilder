...
is_multipath_part () {
	...
	if [ $(multipath -l $mp | grep -v "DM multipath kernel driver not loaded" | wc -l) -gt  0 ]; then
		return 0
	fi
	return 1
}
...
humandev () {
	...
	/dev/mapper/*)
		...
		wwid=$(multipath -l ${device} | grep -v "DM multipath kernel driver not loaded" | head -n 1 | sed "s/^${device} \+(\([^)]\+\)).*/\1/")
