...
class PageBase(plugin.PluginUI):
    ...
    # kylin add
    def set_checkbox_text(self, contents):
        pass

    def set_kylin_custom_text(self, contents):
        pass
    # kylin end
...
class PageGtk(PageBase):
    ...
    # kylin add
    def on_data_checkbutton_clicked(self, w):
        if self.data_checkbutton.get_active():
            if not self.config.has_section("lixiang"):
                self.config.add_section("lixiang")
            self.config.set("lixiang", "data", "1")
        else:
            if not self.config.has_section("lixiang"):
                self.config.add_section("lixiang")
            self.config.set("lixiang", "data", "0")
        self.config.write(open('/tmp/kylin-data.ini', "w"))

    def on_backup_checkbutton_clicked(self, w):
        if self.backup_checkbutton.get_active():
            if not self.config.has_section("lixiang"):
                self.config.add_section("lixiang")
            self.config.set("lixiang", "backup", "1")
        else:
            if not self.config.has_section("lixiang"):
                self.config.add_section("lixiang")
            self.config.set("lixiang", "backup", "0")
        self.config.write(open('/tmp/kylin-data.ini', "w"))
    # kylin end
    ...
    def set_checkbox_text(self, contents):
        ...
        # text = self.controller.get_string('ubiquity/text/installation_others')
        # self.other_label.set_text(text)
        if 'data' in contents:
            self.data_checkbutton.set_label(contents['data'].title)
            self.datalabel.set_markup(fmt % contents['data'].desc)
        if 'backup' in contents:
            self.backup_checkbutton.set_label(contents['backup'].title)
            self.backuplabel.set_markup(fmt % contents['backup'].desc)

    def set_kylin_custom_text(self, contents):
        arch, subarch = archdetect()

        tmpoutput = os.popen('cat /proc/cpuinfo | grep -i "model name" | head -n1 | grep 1500', 'r')
        is_ft1500 = tmpoutput.read()[:-1]

        if arch in ('arm64', 'armhf') and is_ft1500 != "":
            text = self.controller.get_string('ubiquity/text/partition_create_tip_arm_label')
        elif arch in ('mips64', 'mips64el'):
            text = self.controller.get_string('ubiquity/text/partition_create_tip_mips_label')
        else:
            text = self.controller.get_string('ubiquity/text/partition_create_tip_label')
        self.part_advanced_kylin_message.set_text(text)
    # kylin end
...
# 
        ...
        if os.path.isfile(filename):
            ...
            if self.config.has_section("lixiang"):
                if self.config.has_option("lixiang", "data"):
                    tmp_value = self.config.get("lixiang", "data")
                    if tmp_value == "1":
                        data_value = True
                    tmp_value = None
                if self.config.has_option("lixiang", "backup"):
                    tmp_value = self.config.get("lixiang", "backup")
                    if tmp_value == "1":
                        backup_value = True
        self.backup_checkbutton.set_active(backup_value)
        self.data_checkbutton.set_active(data_value)
        len_widgets = 0
        # kylin end
        ticked = False
        for option, name in option_to_widget:
            ...
            if option in options:
                ...
                #     ticked = True
              	### aarch64, mips64, mips64el no show resize
                if self.arch_version == "aarch64" and  option == 'resize' and name == 'resize_use_free':
                    opt_widget.hide()
                    opt_desc.hide()
                elif self.arch_version == "mips64" and  option == 'resize' and name == 'resize_use_free':
                    opt_widget.hide()
                    opt_desc.hide()
                elif self.arch_version == "mips64el" and  option == 'resize' and name == 'resize_use_free':
                    opt_widget.hide()
                    opt_desc.hide()
        ...
        # self.use_crypto.set_visible(False)
        # self.use_crypto_desc.set_visible(False)
        # self.use_lvm.set_visible(False)
        # self.use_lvm_desc.set_visible(False)
        self.advanced_features_button.set_visible(False)
        self.advanced_features_selected.set_visible(False)
        self.advanced_features_desc.set_visible(False)

        self.backup_checkbutton.set_active(True)
        # self.backup_checkbutton.set_sensitive(False)
        self.data_checkbutton.set_active(True)

        # just show backup
        # if not os.path.exists("/rofs/usr/bin/kybackup"):
        #     self.backup_checkbutton.hide()
        #     self.backuplabel.hide()
        #     self.backup_checkbutton.set_active(False)
        # kylin end
    ...
    def on_partition_use_combo_changed(self, combobox):
        ...
        # kylin add, 非 swap 和 efi 默认勾选格式化
        self.partition_edit_format_checkbutton.set_active(False)
        if show and mount_model:
            # kylin add, bug 15352, format is selected by default for non-swap and efi
            self.partition_edit_format_checkbutton.set_active(True)
            self.partition_dialog_okbutton.set_sensitive(True)
            fs = model[iterator][1]
            mount_model.clear()
    ...
    # kylin add
    @plugin.only_this_page
    def on_partition_mount_combo_changed(self, combobox):
        mountpoint = self.partition_mount_combo.get_child().get_text()
    ...
    def update_partman(self, disk_cache, partition_cache, cache_order):
        ...
        for item in cache_order:
            # kylin add, kylin disable show docker disk
            if "=dev=mapper=docker-" in item:
                continue
            # kylin end
...
class Page(plugin.Plugin):
    ...
    def get_contents(self):
        contents = {}
        q = 'ubiquity/partitioner/data'
        title = self.description(q)
        desc = self.extended_description(q)
        contents['data'] = PartitioningOption(title, desc)
        q = 'ubiquity/partitioner/backup'
        title = self.description(q)
        desc = self.extended_description(q)
        contents['backup'] = PartitioningOption(title, desc)
        return contents
    # kylin end
    ...
    def run(self, priority, question):
        ...
        if question.endswith('automatically_partition'):
            ...
            self.ui.set_checkbox_text(contents)
            self.ui.set_kylin_custom_text(contents)
            # kylin end
